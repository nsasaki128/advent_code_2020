...
これらを塊ごとに足し合わせる。
### Part Two
行ごとに存在する文字のsetを作り、塊内でそれらのandをとればよい。
...
単純に同じ処理すると、雑に見積もると10^6 \* 10^7 という恐ろしい計算量になる
...
### Part One
裏返すタイルの位置に関しては、入力文字の手前から条件に合致するものを探していけば良い

位置関係さえわかれば良いので、丁寧に実際の座標とはせず、マンハッタン距離が2になるように各移動を定義すれば良い

座標をキーとし、タイルを裏返した数をバリューとするマップを定義し、最終的にバリューが奇数のものの総和が黒くなっているタイルの総数となる

### Part Two
黒いタイルの座標を持つsetを用意して、黒いタイルの座標とその周辺座標に関して、翌日には黒い座標になるかを調べる

今現在調査している座標のタイルが黒いのかどうかに気をつければ良い
...
### Part One
7^(N) % 20201227 = card\_id

7^(M) % 20201227 = door\_id

となる、NとMを求める

card\_idとMの組み合わせおよびdoor\_idとNの組み合わせから一意の暗号キーを求める必要がある

7も20201227も素数なのがポイントで1以上20201227未満の数ならば必ずループ数は求まる

また、このような2数に関して必ず一意に決まる暗号キーは存在する

答えを言ってしまえば 7^(MN) % 20201227が暗号キー

(card\_id)^M % 20201227 = 7^(MN) % 20201227

(door\_id)^N % 20201227 = 7^(MN) % 20201227

となりいずれも同じ値になる

### Part Two
ここから先は自分の目で確かめよう！（昔の攻略本風）
